...
const formula: Record<number | string, Formula> = {
  1: { name: '基础作战记录', minutes: 45, weight: 2 },
  2: { name: '初级作战记录', minutes: 60 + 20, weight: 3 },
  3: { name: '中级作战记录', minutes: 60 * 3, weight: 5 },
  4: { name: '赤金', minutes: 60 + 12, weight: 2 },
...
const currentWeight = computed(() => {
  /** 单位制作的时间 */
  const productPreMinutes = currentFormula.value.minutes / props.data.speed

  /** 运行时长 */
  const runningDuration = intervalToDuration({
    start: new Date(props.data.lastUpdateTime * 1000),
    end: new Date(),
  })
...
<template>
  <Base title="制造站" :level="data.level" color="#FFD800">
    ...
    <template #info>
      <div w-full flex="~ items-center justify-between">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            fill-rule="evenodd" clip-rule="evenodd"
            d="M9.42857 2.28571L3.42857 5.71429L3.42857 12.5714L9.42857 16L15.4286 12.5714V5.71429L9.42857 2.28571ZM9.42857 2.85714L4 6L9.31754 9.21143L9.42857 15.4286L14.8571 12.2857V6L9.42857 2.85714Z"
            fill="#FFD802"
          />
        ...
        <span>
          <span class="text-#ffd800">{{ currentWeight }}</span>/{{ data.capacity }}
        </span>
      </div>
    ...
    <ResidentCharacter v-for="i in data.chars" :key="i.charId" :resident="i" />
  </Base>
</template>
