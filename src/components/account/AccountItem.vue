...
<template>
  <section bg="list-item" relative h-70px p-10px>
    ...
    <div relative flex="~ gap-1 items-center">
      <Avatar v-if="arknightRole.info?.status.avatar" :avatar="arknightRole.info?.status.avatar" />
      <div flex="~ col 1">
      ...
      <div flex="~ justify-end gap-1">
        <button
          v-if="arknightRole.currentUid !== account.uid"
          type="button"
          p="x-1 y-1px"
          bg="white hover:primary" c="black hover:white"
          transition-colors
          @click="arknightRole.setCurrentUid(account.uid)"
        >
          设为默认
        </button>
        <button
          ...
          transition-colors
          @click="arknightRole.removeRole(account.uid)"
        >
