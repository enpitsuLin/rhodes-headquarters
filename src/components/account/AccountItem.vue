...
import type { ArknightRole } from '@/store/schema'
import Avatar from '~/components/Avatar.vue'

defineProps<{ account: ArknightRole, current: boolean }>()
defineEmits<{
  delete: [uid: ArknightRole['uid']]
}>()

const { currentUid, info } = storeToRefs(useArknightRole())
</script>
...
<template>
  <section bg="list-item" relative h-70px p-10px>
    ...
    <div relative flex="~ gap-1 items-center">
      <Avatar v-if="info?.status.avatar" :avatar="info?.status.avatar" />
      <div flex="~ col 1">
