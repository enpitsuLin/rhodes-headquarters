export const ACCOUNTS_KEY = 'skland-webext:accounts'
export const DEFAULT_ACCOUNT_ID_KEY = 'skland-webext:default-account-id'

export const PLAYER_UID_KEY = 'skland-webext:player-uid'
export const PLAYER_CACHE_KEY = 'skland-webext:player-cache-uid'

export const STORAGE_KEY_USERS = 'skland-webext:users'
export const STORAGE_KEY_CURRENT_USER_ID = 'skland-webext:current-user-id'

export const STORAGE_KEY_CURRENT_ROLE_UID = 'skland-webext:current-role-id'
export const STORAGE_KEY_ROLE_INFO = 'skland-webext:role-info'

export const PORT_NAME = 'skland-webext-port'
